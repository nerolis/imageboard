    ...
    <Item.Content as='h7'>
        ...
        {thread.text} 
          ...
          <Link to={`/threads/${thread.id}`}>
           ...
           <Button disabled={this.state.inactive} floated='right' color='black' compact onClick={() => upvoteThread(thread.id).then(this.setState({inactive: true}))}>Like: {thread.like}</Button>
        </Message>
        </Message>
        <Message color='brown'>
        <PostList 
